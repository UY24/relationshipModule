# backend/app/services/relationship/scrapedo_ai_client.py
...
Pooled client, backoff and redaction helpers come from
``app.services.common.scrapedo_http`` — same vendor, same retry semantics.
...
from app.services.relationship.outcomes import categorize_http_error
from app.services.common.scrapedo_http import (
    CREDITS_PER_CALL,
    _backoff_seconds,
    _get_shared_client,
    _redact,
...
async def search_ai_mode(
    ...
    for attempt in range(attempts):
        ...
        try:
            request_count += 1
            # Account-wide scrape.do gate, shared with AI Mode. Wraps only the
