# backend/app/services/relationship/relationship_outputs.py
...
from app.services.relationship.cost import calculate_gemini_cost_usd
from app.services.relationship.modes.relationship import (
    ai_mode_arrays,
    build_row_result,
    row_fields,
)
from app.services.relationship.reporting import (
    retry_column,
    retry_row,
    s3_passthrough,
)
...
def _passthrough_fieldnames(header: list[str]) -> list[tuple[str, str]]:
    ...
    module's reserved/computed names. The rule itself is shared with AI Mode —
...
def _write_outputs(prefix: str, counters: store.Counters,
    ...
    for original in store.iter_input_rows(prefix):
        ...
        # Only rows with nothing to show for their credits. A verdict
    ...
    summary = {
        ...
        "cost": {
            # scrape.do bills CREDITS, never dollars. No per-search USD key appears —
